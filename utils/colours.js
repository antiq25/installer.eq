...
function generateGradientCSS(className, colors) {
  const colorValues = colors.map(color => `"${color}"`).join(', ');
  return `.${className} {
  background: linear-gradient(45deg, ${colorValues});
}`;
}

for (const scheme of colorSchemes) {
  const className = `linear-gradient-${scheme.name.toLowerCase().replace(/\s+/g, '-')}`;
  const cssCode = generateGradientCSS(className, scheme.colors);
  console.log(cssCode);
}
